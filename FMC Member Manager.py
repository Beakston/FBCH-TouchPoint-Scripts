...
WHERE 
    dbo.Program.Id = 1108 --AND EXISTS (Select CheckInTimes_alias1.CheckInTime From dbo.CheckInTimes Where dbo.People.PeopleId = CheckInTimes_alias1.PeopleId)
...
<a href="'''+ model.CmsHost + '''/PyScript/FMC%20Balance"><i class="fa fa-usd fa-2x" class=”button-solid”></i></a>&nbsp;&nbsp;&nbsp;&nbsp;
<a href="'''+ model.CmsHost + '''/PyScript/FMC%20Reports"><i class="fa fa-stack-exchange fa-2x" aria-hidden="true"></i></a>
<style>    
button-solid {
  padding: 5px 10px;
...
for a in q.QuerySql(fmclistsql):

    print ('<tr role = "row">')
    print ('<td role = "cell"> {} </td>').format(a.FamilyId)
    print ('<td role = "cell"><a href="') + model.CmsHost + '/PyScript/FMC%20Member%20Details?p1={1}&FamilyId={3}&ProgramName={4}&ProgramID={5}"> {0} ({2}) </a></td>'.format(a.Name, a.PeopleId, a.Age, a.FamilyId, ProgramName, ProgramID)
...
if model.HttpMethod == 'post' and model.Data.a == "send":  # Posting a message to be sent
    ...
    print model.SendSms('PeopleId = {}'.format(model.Data.to), sendGroup, "Open Invoice", model.Data.message)
    print model.Email('PeopleId = {}'.format(model.Data.to),3134, "bswaby@fbchtn.org", "Ben Swaby - FBCHville", "Outstanding FMC Fees", model.Data.message)
